...
4. RLS do Postgres entra como segunda tranca no portão de abertura pública (SPEC secao 9.4) - adiada
...
- Divisão de parcelas nunca arredonda parcela a parcela: calcula N-1 parcelas e a última recebe o
  resto (ou distribui os centavos excedentes nas primeiras), garantindo soma parcelas = total ao centavo.
...
|---|---|---|
| Render | free tier dorme após 15 min (cold start 30-60 s) e o Postgres grátis de 256 MB **expira em 90 dias**; para valer, ~US$ 14/mês (web US$ 7 + PG US$ 7) | ~3x o custo do Railway já pago, sem vantagem |
...
**Documentos afetados:** `SPEC.md` atualizada (D1, D2, D9 apontam para cá; modelo de dados secao 7 migrou

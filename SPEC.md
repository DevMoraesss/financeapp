...
| D4 | **Saldo é sempre derivado** (`saldo_inicial + Σ transações`), nunca armazenado. Divergência com o banco real se corrige com **transação de ajuste**. | Saldo armazenado dessincroniza silenciosamente (causa nº 1 de bug nesses apps). No volume esperado, a soma custa milissegundos. |
...
| D8 | **Parcelamento na v1, versão simples:** lançar em N× cria as N transações de uma vez, ligadas por grupo. | Com fatura na v1, fatura sem parcela não representa a fatura real de quase ninguém no Brasil. A versão simples é barata (um loop). |
...
| D10 | **Importação CSV/OFX, Metas e Investimentos: v2** (ver §5). | Cada um é um subsistema. A v1 já assumiu fatura + recorrência + parcelas; esses são os cortes mais seguros. |
| D11 | **LGPD na v1: exclusão de conta (hard delete) + export de dados.** Política de privacidade e backup testado são **pré-requisitos do portão de abertura pública** (§9.4), não da v1. | Exclusão e portabilidade são direitos do titular; implementar depois é pior. Política/backup só se tornam críticos quando entram desconhecidos. |
...
- **Dashboard reduzido**: saldo total e por conta, receitas × despesas do mês, donut de despesas por categoria do mês, últimas transações.
...
| Importação CSV/OFX | v2 | O difícil é dedup, não parse. `transacao.id_externo` já nasce na v1 para isso (§7). |
...
- **Saldo atual (derivado)** = `saldo_inicial` + Σ transações da conta com `status = confirmada` **e** `data ≤ hoje`.
...
- Status: `confirmada` (padrão em lançamento manual) ou `pendente` (só gerada por recorrência, §5.5).
...
- **A fatura não é uma tabela — é derivada** das transações do cartão pelo ciclo:
  - A fatura identificada por `AAAA-MM` **vence** no `dia_vencimento` do mês `MM` e **fecha** no `dia_fechamento` do mês `MM`.
  - Ela contém as despesas do cartão com `data` no intervalo: **do dia seguinte ao fechamento do mês anterior até o dia do fechamento do mês MM** (inclusive).
  - Exemplo (fecha dia 03, vence dia 10): fatura **09/2026** cobre compras de 04/08/2026 a 03/09/2026, fecha em 03/09/2026 e vence em 10/09/2026.
...
- Ao lançar despesa parcelada ("R$ 3.600,00 em 12×"), o sistema cria as N transações de uma vez:
  - Cada parcela com `valor = total / N`, arredondado a 2 casas com o resto distribuído nas primeiras parcelas — R$ 33,34 + R$ 33,33 + R$ 33,33 para R$ 100,00 em 3× — garantindo Σ parcelas = total ao centavo.
...
- Progresso do mês = Σ despesas `confirmadas` da categoria com data no mês calendário corrente ÷ limite.
...
- Dado cartão que fecha dia 03 e vence dia 10, quando lanço compra em 04/08, então ela aparece na fatura 09/2026; compra em 02/08 aparece na fatura 08/2026 (§5.3).
...
Como usuário, quero lançar "R$ 300,00 em 3×" e ver cada parcela na fatura certa.
...
- Total que não divide exato (R$ 100,00 em 3×) distribui o resto nas primeiras parcelas e a soma bate o total ao centavo.
...
- Dado saldo no app R$ 1.230,00 e no banco R$ 1.198,00, quando informo R$ 1.198,00 em "Ajustar saldo", então é criada despesa de R$ 32,00 na categoria "Ajuste" com data de hoje e o saldo passa a R$ 1.198,00. Nenhuma transação antiga é alterada.
...
Decisão 2). `criado_em`/`atualizado_em` em tudo. Um schema Postgres por módulo (`docs/arquitetura.md` §2).
...
**O que NÃO existe de propósito:** tabela de saldo (D4), tabela de fatura (derivada — §5.3),
...
- **"Hoje" injetável**: toda regra que depende da data atual (saldo, fatura, recorrência) lê de um provedor de relógio que, em desenvolvimento/teste, aceita override (ex.: env `TEST_TODAY=2026-09-04`). Sem isso a verificação §10 é impossível de rodar.
...
Datas assumem "hoje" = **15/08/2026**, avançado via `TEST_TODAY` (§9.5). Conferir cada valor exato.
...
| 6 | Lançar no cartão "Fone" R$ 300,00 em 3×, 14/08 | 3 parcelas de R$ 100,00: 1/3 na fatura 09/2026, 2/3 na 10/2026, 3/3 na 11/2026; cartão hoje: **−R$ 100,00**; despesas de agosto: **R$ 250,00** |
...
- **Sistema:** Ajuste (tipo conforme o sinal do ajuste; não editável, não arquivável)
